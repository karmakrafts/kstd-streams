...
namespace kstd::streams {
    ...
    struct IdentityPipe : public BasicPipe<S> {
        ...
        template<typename P>
        [[nodiscard]] constexpr auto filter(P&& predicate) noexcept -> decltype(auto) {
            static_assert(std::is_convertible_v<P, std::function<bool(out_type&)>>, "Closure signature doesn't match");
            return Pipe(std::move(*this), [&predicate](auto& supplier) -> Option<out_type> {
        ...
        template<typename M>
        [[nodiscard]] constexpr auto map(M&& mapper) noexcept -> decltype(auto) {
            ...
            return Pipe(std::move(*this), [&mapper](auto& supplier) -> Option<return_type> {
                auto element = supplier.get_next();
                if(!element) {
                    return {};
                }

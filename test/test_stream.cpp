...
TEST(kstd_streams_Stream, TestCollectFromMap) {
    ...
    const auto keys = kstd::streams::stream(entries)
                              .map([](auto& x) {
                                  return x.first;
                              })
                              .collect<std::vector>();
    ...
    const auto values = kstd::streams::stream(entries)
                                .map([](auto& x) {
                                    return x.second;
                                })
                                .collect<std::vector>();
...
TEST(kstd_streams_Stream, TestSorted) {
    ...
    const auto result = kstd::streams::stream(test_values)
                                .sorted([](const auto& a, const auto& b) {
                                    return a > b;
                                })
                                .collect<std::vector>();
    // @formatter:on
...
TEST(kstd_streams_Stream, TestSortedDefault) {
    // @formatter:off
    const auto result = kstd::streams::stream(test_values).sorted().collect<std::vector>();
    // @formatter:on
...
TEST(kstd_streams_Stream, TestAnyMatch) {
    ASSERT_TRUE(kstd::streams::stream(test_values).any_match([](float x) {
        return x > 1.0F;
    }));

    ASSERT_FALSE(kstd::streams::stream(test_values).any_match([](float x) {
        return x < 0.0F;
    }));
}

TEST(kstd_streams_Stream, TestAllMatch) {
    ASSERT_TRUE(kstd::streams::stream(test_values).all_match([](float x) {
        return x > 0.0F;
    }));

    ASSERT_FALSE(kstd::streams::stream(test_values).all_match([](float x) {
        return x > 2.0F;
    }));
}

TEST(kstd_streams_Stream, TestNoneMatch) {
    ASSERT_TRUE(kstd::streams::stream(test_values).none_match([](float x) {
        return x > 5.0F;
    }));

    ASSERT_FALSE(kstd::streams::stream(test_values).none_match([](float x) {
        return x > 0.0F;
    }));
}
...
TEST(kstd_streams_Stream, TestForEach) {
    ...
    kstd::streams::stream(test_values).for_each([&index](float x) {
        ASSERT_EQ(x, test_values[index++]);
    });
}

TEST(kstd_streams_Stream, TestForEachIndexed) {
    kstd::streams::stream(test_values).for_each_indexed([](float x, auto i) {
...
TEST(kstd_streams_Stream, TestSkip) {
    ...
    // @formatter:off
    const auto result = kstd::streams::stream(test_values).skip(2).collect<std::vector>();
    // @formatter:on
...
TEST(kstd_streams_Stream, TestPreChain) {
    ...
    const auto chained_count = expected.size();
    auto result =
            (kstd::streams::stream(test_values).pre_chain(kstd::streams::stream(test_values))).collect<std::vector>();
...
TEST(kstd_streams_Stream, TestReduce) {
    // @formatter:off
    auto result = kstd::streams::stream(test_values).reduce([](auto a, auto b) {
        return a * b;
    });
    // @formatter:on
...
TEST(kstd_streams_Stream, TestSum) {
    auto result = kstd::streams::stream(test_values).sum();
    ASSERT_TRUE(result);
...
TEST(kstd_streams_Stream, TestCollectMap) {
    ...
    // @formatter:off
    auto result = kstd::streams::stream(test_values)
                          .collect_map<std::unordered_map>(
                                  [&index](auto& x) {
                                      return index++;
                                  },
                                  [](auto& x) {
...
TEST(kstd_streams_Stream, TestFilter) {
    ...
    const auto result = kstd::streams::stream(test_values)
                                .filter([](auto& x) {
                                    return (static_cast<int32_t>(x) & 1) == 0;
                                })
                                .collect<std::vector>();
    // @formatter:on
...
TEST(kstd_streams_Stream, TestFilterNotNull) {
    ...
    // @formatter:off
    const auto result = kstd::streams::stream(pointers).filter_not_null().collect<std::vector>();
    // @formatter:on
...
TEST(kstd_streams_Stream, TestMax) {
    auto result = kstd::streams::stream(test_values).max();
    ASSERT_TRUE(result);
    ASSERT_EQ(*result, 4.5F);
}
...
TEST(kstd_streams_Stream, TestLimit) {
    ...
    // @formatter:off
    const auto result = kstd::streams::stream(test_values).limit(max_count).collect<std::vector>();
    // @formatter:on
